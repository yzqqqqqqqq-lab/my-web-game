...
export default function CasinoGamePlayPage({
  ...
  const handleMobilePlay = (demo: boolean) => {
    const params = new URLSearchParams();
    params.set("mode", demo ? "demo" : "real");
    if (mobileFullscreen) {
  ...
  return (
    <div className=" bg-grey-600 pb-8 overflow-x-hidden">
      <main className="space-y-6">
        <div className="container mx-auto">
          <div className="flex flex-col gap-6">
            {/* Game Wrapper - Hidden on mobile */}
            <div
              className="game-wrapper w-full mt-10 hidden md:block"
              data-testid="game-active"
            >
              <div
                className="game-wrapper w-full max-w-[1200px] mx-auto rounded-md overflow-hidden"
                style={
                  {
                    "--max-height": "671px",
                    "--fullscreen-z": "9999",
              ...
              >
                <div
                  id={containerId}
                  className="game-content relative w-full"
                  style={
                    {
                ...
                >
                  <div className="flex items-center justify-between px-4 py-3 bg-grey-700 rounded-b-md border-t border-grey-700">
            ...
            <div className="game-meta bg-grey-700 rounded-lg p-6 w-full max-w-[1200px] mx-auto  mt-[3vw] md:mt-0">
              {/* Mobile Layout - Show on mobile only */}
              <div className="md:hidden">
                <div className="mobile-header mb-4">
                  <div className="flex gap-4 mb-4">
                    {/* Game Thumbnail */}
                    <div className="img-wrap shrink-0">
                      <Image
                        src={game.cover}
                        alt={game.title}
                        width={280}
                        ...
                        style={{
                          maxWidth: "280px",
                          maxHeight: "375px",
                          aspectRatio: "0.746667",
                          width: "100%",
                        }}
                      />
                    </div>
                    {/* Game Info */}
                    <div className="info-wrap relative flex-1 min-w-0">
                      <div className="main-info">
                        ...
                        <h2 className="text-base mb-1">
                          <Link
                            ...
                            className="text-grey-200 hover:text-white transition-colors"
                          >
                            {game.provider}
                          </Link>
                        </h2>
                  ...
                  {/* Currency and Fullscreen Toggle */}
                  <div className="flex flex-col gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="dropdown flex">
                        <button
                          type="button"
                          className="flex items-center gap-1 text-white hover:text-grey-200 transition-colors"
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        className="sr-only"
                        data-testid="mobile-game-fullscreen-toggle"
                      />
                      <div
                        className={`relative w-11 h-6 rounded-full transition-colors ${
                          mobileFullscreen ? "bg-green-500" : "bg-grey-600"
                        }`}
                      >
                        <div
                          className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${
                            mobileFullscreen ? "translate-x-5" : "translate-x-0"
                          }`}
                        />
                      </div>
                      <span className="text-base text-grey-200">
                        以全屏进行游戏
                      </span>
                    </label>
                  </div>
                  ...
                  {/* Play Buttons */}
                  <div className="mobile-play-buttons flex gap-2">
                    <button
                      type="button"
                      onClick={() => handleMobilePlay(true)}
                      className="flex-1 py-2 px-4 rounded-md bg-grey-400 text-white font-semibold text-sm shadow-md hover:bg-grey-300 transition-colors"
                      data-testid="fun-play-button"
                    >
                      试玩模式
                    </button>
                    <button
                      type="button"
                      onClick={() => handleMobilePlay(false)}
                      className="flex-1 py-2 px-4 rounded-md bg-green-500 text-black font-semibold text-sm shadow-md hover:bg-green-400 transition-colors"
                      data-testid="real-play-button"
                    >
                      真钱模式
                    </button>
                  </div>
